...
class Scanner:
	...
	def scanToken(self):
		...
		#two character lexemes, here peekAndMatch consumes the current token and forward if matched
			...
			if reserved_keyword:= reserved_keywords[literal] :
				self.addToken(reserved_keyword, '') # we explicitly add '' since we do not want to depend upon autogenerated values
	...
	def addToken(self, token, literal=None):
		...
		self.token_list.append(
			Token(token, 
				  ...
				  self.getLiteral() if literal==None  else literal,
				  self.line)
			)
